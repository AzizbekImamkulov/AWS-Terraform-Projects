...
resource "aws_api_gateway_integration" "create_integration" {
  ...
  integration_http_method = "POST" # ✅ Правильно
  type                    = "AWS_PROXY"
...
resource "aws_api_gateway_integration" "redirect_integration" {
  ...
  integration_http_method = "GET" # ✅ Правильно
  type                    = "AWS_PROXY"
...
resource "aws_api_gateway_integration" "create_options_integration" {
  rest_api_id          = aws_api_gateway_rest_api.url_api.id
  resource_id          = aws_api_gateway_resource.create_path.id
...
resource "aws_api_gateway_method_response" "create_options_response" {
  rest_api_id = aws_api_gateway_rest_api.url_api.id
  resource_id = aws_api_gateway_resource.create_path.id
  http_method = aws_api_gateway_method.create_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "create_options_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.url_api.id
  resource_id = aws_api_gateway_resource.create_path.id
  http_method = aws_api_gateway_method.create_options.http_method
  status_code = "200"
  depends_on  = [aws_api_gateway_integration.create_options_integration]
  response_parameters = {
...
resource "aws_api_gateway_integration_response" "redirect_options_integration_response" {
  ...
  http_method = aws_api_gateway_method.redirect_options.http_method
  status_code = "200"
  depends_on  = [aws_api_gateway_integration.redirect_options_integration]
  response_parameters = {
    "method.response.header.Access-Control-Allow-Headers" = "'Content-Type,X-Amz-Date,Authorization,X-Api-Key'"
...
resource "aws_api_gateway_deployment" "url_api_deployment" {
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.create_options_integration,
    aws_api_gateway_method.redirect_options,
    aws_api_gateway_integration.redirect_options_integration
  ]

  triggers = {
    redeployment = timestamp() # ⏱ всегда перезапускает deployment
  }
}
